...
function diffLetter(str1, str2){
  var newNumb = parseInt(str2.charCodeAt(0)) - parseInt(str1.charCodeAt(0));
  if (newNumb < 0){
    newNumb = -newNumb;
  }
  return newNumb;
...
function diffNumb(str1, str2){
  var newNumb = parseInt(str2.charAt(1)) - parseInt(str1.charAt(1));
  if (newNumb < 0){
    newNumb = -newNumb;
  }
  return newNumb;
...
function calcDistance(){
  var newLetter = diffLetter(arguments[0], arguments[1]);
  var newNumb = diffNumb(arguments[0], arguments[1]);
  var answer = Math.sqrt(Math.pow(newLetter, 2) + Math.pow(newNumb, 2));
...
function evaluateRoute(){
  ...
  for (var i = 0; i < arguments[0].length; i++){
    if (isRock(arguments[0][i])){
      counterR++;
    } else if (isCurrent(arguments[0][i])){
      counterC++;
    }
  }
  if (counterR !== 0 || counterC > 2){
    return false; 
  } else {
    return true;
  }
